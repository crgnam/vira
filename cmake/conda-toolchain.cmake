# conda-toolchain.cmake - Cross-platform conda toolchain
# Makes conda packages work with CMakeLists.txt files by setting up proper search paths
...
if(UNIX AND NOT APPLE)
    ...
    if(EXISTS "${CONDA_GCC}" AND EXISTS "${CONDA_GXX}")
        message(STATUS "Using conda compilers: ${CONDA_GCC}, ${CONDA_GXX}")
        set(CMAKE_C_COMPILER "${CONDA_GCC}")
        set(CMAKE_CXX_COMPILER "${CONDA_GXX}")
        # Clear conda's default cross-compilation flags that may interfere
        set(CMAKE_EXE_LINKER_FLAGS_INIT "")
        set(CMAKE_SHARED_LINKER_FLAGS_INIT "")
        set(CMAKE_MODULE_LINKER_FLAGS_INIT "")
    else()
        message(STATUS "Conda compilers not found, falling back to system compilers")
        ...
        if(SYSTEM_CC AND SYSTEM_CXX)
            ...
            message(WARNING "Could not find system compilers. You may need to install build-essential or similar.")
        endif()
    endif()
...
elseif(APPLE)
    ...
    if(EXISTS "${CONDA_CLANG}" AND EXISTS "${CONDA_CLANGXX}")
        message(STATUS "Using conda compilers: ${CONDA_CLANG}, ${CONDA_CLANGXX}")
        set(CMAKE_C_COMPILER "${CONDA_CLANG}")
        set(CMAKE_CXX_COMPILER "${CONDA_CLANGXX}")
    else()
        # Try generic conda clang as fallback
        set(CONDA_CLANG "${CONDA_PREFIX}/bin/clang")
        set(CONDA_CLANGXX "${CONDA_PREFIX}/bin/clang++")
        
        if(EXISTS "${CONDA_CLANG}" AND EXISTS "${CONDA_CLANGXX}")
            message(STATUS "Using generic conda compilers: ${CONDA_CLANG}, ${CONDA_CLANGXX}")
            set(CMAKE_C_COMPILER "${CONDA_CLANG}")
            set(CMAKE_CXX_COMPILER "${CONDA_CLANGXX}")
        else()
            message(STATUS "Conda compilers not found, falling back to system compilers")
            ...
            if(SYSTEM_CC AND SYSTEM_CXX)
                ...
                message(WARNING "Could not find system compilers. You may need to install Xcode command line tools.")
            endif()
        endif()
    endif()
    
elseif(WIN32)
    # Windows: conda packages typically use system MSVC, so usually no action needed
    # Could add conda MSVC detection here if needed in the future
    message(STATUS "Windows: using system compilers (conda packages typically compatible with system MSVC)")
endif()
